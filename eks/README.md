...
catsndogs.lol uses Docker containers to host our application. Until today we’ve run everything on a spare laptop, but now we’re moving to the Amazon Elastic Container Service for Kubernetes. Our DevOps Shepherd wants to take advantage of the latest and greatest features of Kubernetes. We also have several new initiatives that the developers and data science teams are keen to release.
As the new DevOps team, you will create the Kubernetes environment, deploy the cats and dogs applications, cope with our hoped-for scaling issues, and enable the other teams to release new features to make our customers happier than ever.
**Welcome aboard!**
...
- This workshop can be run in any of the following AWS regions: US East (N. Virginia) or US West (Oregon).
